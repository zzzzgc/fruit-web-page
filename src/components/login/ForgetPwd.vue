<template>
    <div class="forget-pwd">
      ...
      <div class="content">
        ...
        <mt-field label="验 证 码" v-model="msgValidCode" placeholder="请输入验证码" style="color:red;" class="horizontal">
  ...
  export default {
    ...
    methods: {
      ...
      resetPwdForForget: function () {
        ...
        this.$http.post('/login/forgetPwd', {phone: this.phone, password: this.password, msgValidCode: this.msgValidCode, verifyCodeType: 2}).then(
          (response) => {
            this.$toast('重置完成请重新登录')
